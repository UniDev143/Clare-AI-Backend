...
const analyseSkin = async (imageData, mimeType = 'image/jpeg', questionnaire = null) => {
...
Base ALL severity ratings purely on visual evidence. Questionnaire context only informs the aiSummary tone.`;
  ...
  console.log(`[SkinAnalysis] Calling Anthropic API (prompt v${PROMPT_VERSION})`);
  const startTime = Date.now();

  try {
    const response = await client.messages.create({
      model:      'claude-sonnet-4-6',   // upgrade to claude-opus-4-6 for production
      max_tokens: 1024,
    ...
    console.log(`[SkinAnalysis] Response in ${elapsed}ms | tokens: ${tokens?.input_tokens}in / ${tokens?.output_tokens}out`);
    ...
    try {
      ...
      console.error('[SkinAnalysis] JSON parse failed. Raw response:', rawText.substring(0, 500));
    ...
    if (error.status === 429) throw new Error('API rate limit reached — try again in a moment');
    if (error.status === 400) throw new Error('Image format not supported by API');
    throw error;
  }
};
